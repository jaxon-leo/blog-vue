<template>
  <div class="resources-container">
    ...
    <div class="page-header">
      ...
      <div class="header-right">
        <div class="search-bar">
          <el-input v-model="params.name" placeholder="搜索你感兴趣的资源,按回车键搜索..." prefix-icon="el-icon-search" clearable />
        </div>
    ...
    <div v-for="category in categories" :key="category.id" class="category-section"
      v-if="category.resources?.length > 0">
      ...
      <div class="pagination-box" v-if="category.total > params.pageSize">
        <el-pagination :current-page.sync="categoryPages[category.id]" :page-size="params.pageSize"
    ...
    <!-- 资源详情对话框 -->
    <el-dialog title="资源详情" :visible.sync="detailDialogVisible" width="600px" class="resource-detail-dialog" top="5vh"
...
export default {
  ...
  methods: {
    ...
    handleEnter(event) {
      if (event.key === 'Enter') {
        //阻止默认的回车行为（比如默认提交表单查询）
        event.preventDefault();
        // 若弹框打开的情况下，触发验证码验证操作， 否则触发搜索
        if (this.detailDialogVisible === true) {
          this.handleVerify()
        } else if (this.detailDialogVisible === false) {
          this.handleSearch()
        }
      }
    },
